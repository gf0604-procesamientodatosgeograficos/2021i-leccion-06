...
```{r collapse=TRUE}
mi_funcion <- 
  function(argumento_1, argumento_2, argumento_n) {
...
```{r collapse=TRUE}
nota_final <- 
  function(promedio_examenes, promedio_proyectos, promedio_tareas) {
    examenes <- promedio_examenes * 0.5
    proyectos <- promedio_proyectos * 0.4
    tareas <- promedio_tareas * 0.1
    
    return(examenes + proyectos + tareas)
  }
```
...
```{r collapse=TRUE}
nota_final <- 
  function(promedio_examenes, promedio_proyectos=0, promedio_tareas=0) {
    examenes <- promedio_examenes * 0.5
    proyectos <- promedio_proyectos * 0.4
    tareas <- promedio_tareas * 0.1
    
    # Al no llamarse a la función return(), se retorna la última expresión:
    examenes + proyectos + tareas
  }
...
**
Ejercicio 01:  
a. Defina una función con nombre ```imc()``` para calcular el [índice de masa corporal (IMC)](https://es.wikipedia.org/wiki/%C3%8Dndice_de_masa_corporal) de una persona con base en su peso (en kilogramos) y su estatura (en metros).
**