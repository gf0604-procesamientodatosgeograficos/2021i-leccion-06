...
## La sentencia ```if```
La sentencia ```if``` evalúa una condición y ejecuta un bloque de instrucciones si es verdadera. El bloque se delimita con los caracteres de "llaves": ```{}```. 
...
## La sentencia ```else```
Una sentencia ```if``` puede ir seguida de una sentencia ```else```, la cual define un bloque que se ejecuta si la condición es falsa.
Por ejemplo:
...
Una sentencia ```if``` puede ir seguida de una o varias sentencias ```else if```, las cuales evalúan condiciones adicionales.
...
Las sentencias ```else if``` deben escribirse antes de la sentencia ```else```, la cual es siempre la última, si es que está presente. Tanto las sentencias ```else if``` como la sentencia ```else``` son opcionales.
